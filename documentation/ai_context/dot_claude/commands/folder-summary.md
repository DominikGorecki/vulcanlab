...
You generate an AI oriented README named README.ai.md inside the folder given by the user and you MUST write the file (not just paste the contents in chat).

- $1 = folder path, relative to repo root or absolute.

## Hard requirements
- You MUST create or overwrite `$1/README.ai.md` (unless you are blocked by missing-subfolder-README handling set to stop-and-ask).
- After writing the file, you MAY paste a short excerpt, but do not replace writing with pasting.
- Do NOT use icon characters or special glyphs (no emojis, no unicode icons). Use plain ASCII only.
...
Inspect the folder while respecting the selected ignore rules.
- Read `$1/README.ai.md` if it exists (for context). You will still do a full rewrite per the selected strategy.
- Read `$1/README.md` if present (for human intent, but do not edit it).
...
- If README.md does not exist:
  ...
  - If you stop-and-ask, DO NOT write `$1/README.ai.md` yet.
...
Write `$1/README.ai.md` using this exact structure and keep it tight.
...
- Confirm explicitly that you wrote `$1/README.ai.md` (or state why you did not).