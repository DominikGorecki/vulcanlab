...
export default function GenVecSuggestionsPage() {
  ...
  const fetchPrompt = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/chunk/work/${workId}/vec-suggestions/prompt`
      );

      if (!response.ok) {
        if (response.status === 404) {
          throw new Error("Work or sanitized file not found. Generate sanitized file first.");
        }
  ...
  const handleSaveManualResponse = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/chunk/work/${workId}/vec-suggestions/manual`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || "Failed to save vec suggestions");
      }
  ...
  const handleRunPrompt = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/chunk/work/${workId}/vec-suggestions/run`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      <div className="flex items-center justify-center h-screen">
        <div className="text-center space-y-4">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <Button variant="outline" onClick={() => router.push(`/chunk/${workId}`)}>
              Back
            </Button>
          </div>
        </div>
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="border-b bg-card p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Generate Vectorization Suggestions</h1>
      ...
      <Dialog open={copyDialogOpen} onOpenChange={setCopyDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[80vh] flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              Paste the LLM&apos;s response here. Expected format: line_number: SKIP or line_number: VECTORIZE
            </DialogDescription>
          </DialogHeader>
